...
# Your name: Jacob Negroni
# Your student id: 65561418
# Your email: jnegroni@umich.edu
# List who you have worked with on this project: Umang Bhojani
...
def read_csv(file):
    ...
    categories = reader.fieldnames.copy()
    categories.remove("State")
    for row in reader:
        data_dict[row["State"]] = dict()
        for category in categories:
            data_dict[row["State"]][category] = int(row[category])
    csvfile.close()
...
def pct_calc(data_dict):
    ...
    for state in data_dict:
        pct_dict[state] = {}
        for category in data_dict[state]:
            if(category == "State Totals"):
                continue
            pct_dict[state][category] = round(data_dict[state][category]*100/(data_dict[state]["State Totals"]),2)
...
def pct_dif(data_dict1, data_dict2):
    ...
    for state in data_dict1:
        pct_dif_dict[state] = {}
        for category in data_dict1[state]:
            if(category in data_dict2[state.replace('-',' ')] and category != "State Totals"):
                diff = abs(data_dict1[state][category] - data_dict2[state.replace('-',' ')][category])
                pct_dif_dict[state][category] = round(diff,2)
...
def csv_out(data_dict, file_name):
    ...
    with open(file_name, "w", newline="") as fileout:
        ...
        writer.writeheader()
        for state in data_dict:
            writer.writerow({"State":state} | data_dict[state])
...
def max_min(data_dict):
    ...
    for category in data_dict:
        max_min["max"][category] = {}
        max_min["min"][category] = {}
        max_sort = sorted(data_dict[category].items(), key = lambda kv:(kv[1], kv[0]), reverse = True)[:5]
        min_sort = sorted(data_dict[category].items(), key = lambda kv:(kv[1], kv[0]), reverse = False)[:5]
        for (state,per) in max_sort:
            max_min["max"][category][state] = per
        for (state,per) in min_sort:
            max_min["min"][category][state] = per
...
class HWTest(unittest.TestCase):
    ...
    def test_pct_calc(self):
        print(pct_calc({"state":{"demo":5,"State Totals":10}}), 
                         {"state":{"demo": 50.0}})
